...
% Fourier transform.
  ...
  SUP=15;                       % Support of largest filter (must be odd)
  F=zeros(SUP,SUP,NF);
  
  F(:,:,1)=makefilter(SUP,1,1);
  F(:,:,2)=makefilter(SUP,2,1);
  F(:,:,3)=makefilter(SUP,2,2);
  F(:,:,4)=makefilter(SUP,3,1);
  F(:,:,5)=makefilter(SUP,3,2);
  F(:,:,6)=makefilter(SUP,3,3);
  F(:,:,7)=makefilter(SUP,4,1);
  F(:,:,8)=makefilter(SUP,4,2);
  F(:,:,9)=makefilter(SUP,4,3);
  F(:,:,10)=makefilter(SUP,5,1);
  F(:,:,11)=makefilter(SUP,5,2);
  F(:,:,12)=makefilter(SUP,5,3);
  F(:,:,13)=makefilter(SUP,5,4);
return
